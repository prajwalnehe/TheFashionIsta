    ...
    const handleOnlinePayment = async () => {
        ...
        try {
            ...
            const options = {
                ...
                image: "https://thefashionista.com/logo.png",
        ...
        <div className="bg-gray-100 min-h-screen py-8">
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="flex items-center text-sm text-gray-500 mb-6 font-medium">
                    <Link to="/cart" className="hover:text-[#111827] transition-colors">Cart</Link>
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <section className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                            <div className="bg-gray-50 px-6 py-4 flex justify-between items-center border-b border-gray-100">
                                <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                    <span className="flex items-center justify-center w-6 h-6 bg-[#111827] text-white text-xs rounded-full">1</span>
                                    Select Delivery Address
                                </h2>
                                {!showAddressForm && (
                                    <button
                                        onClick={() => setShowAddressForm(true)}
                                        className="text-[#111827] text-sm font-bold uppercase hover:underline"
                                    >
                                        + Add New Address
                                    </button>
                                )}
                                ...
                                {showAddressForm ? (
                                    ...
                                    <form onSubmit={handleAddressSubmit} className="space-y-4 animate-fadeIn">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <input type="text" name="name" placeholder="Full Name *" value={formData.name} onChange={handleInputChange} required className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none" />
                                            <input type="tel" name="phone" placeholder="Phone Number *" value={formData.phone} onChange={handleInputChange} required maxLength="10" className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none" />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <input type="text" name="locality" placeholder="Locality / Area *" value={formData.locality} onChange={handleInputChange} required className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none" />
                                        </div>

                                        <textarea name="address" placeholder="Address (House No, Building, Street) *" value={formData.address} onChange={handleInputChange} required rows="3" className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none resize-none" />

                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <input type="text" name="city" placeholder="City / District *" value={formData.city} onChange={handleInputChange} required className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none" />
                                            <input type="text" name="state" placeholder="State *" value={formData.state} onChange={handleInputChange} required className="input-field p-3 border rounded w-full focus:ring-1 focus:ring-[#111827] outline-none" />
                                        </div>

                                        <div className="flex gap-4 pt-2">
                                            <label className="flex items-center cursor-pointer">
                                                <input type="radio" name="type" value="Home" checked={formData.type === 'Home'} onChange={handleInputChange} className="text-[#111827] focus:ring-[#111827]" />
                                                <span className="ml-2 text-sm text-gray-700">Home</span>
                                            </label>
                                            <label className="flex items-center cursor-pointer">
                                                <input type="radio" name="type" value="Work" checked={formData.type === 'Work'} onChange={handleInputChange} className="text-[#111827] focus:ring-[#111827]" />
                                                <span className="ml-2 text-sm text-gray-700">Work</span>
                                            </label>
                                        </div>

                                        <button type="submit" className="w-full bg-[#111827] text-white font-bold py-3 rounded hover:bg-[#1f2937] transition-colors mt-4">
                                            SAVE ADDRESS
                                        </button>
                                    </form>
                                ) : (
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {addresses.length === 0 ? (
                                            <div className="text-center py-8 text-gray-500">
                                                ...
                                                <button onClick={() => setShowAddressForm(true)} className="mt-2 text-[#111827] font-medium hover:underline">Add one now</button>
                                            </div>
                                        ) : (
                                            addresses.map(addr => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`border rounded-lg p-4 cursor-pointer transition-all relative group ${selectedAddressId === addrId
                                                            ? 'border-[#111827] bg-[#111827]/5 shadow-sm ring-1 ring-[#111827]'
                                                            : 'border-gray-200 hover:border-gray-300'
                                                            }`}
                                                    >
                                                        <div className="flex items-start gap-3">
                                                            <div className={`mt-1 w-5 h-5 rounded-full border flex items-center justify-center ${selectedAddressId === addrId ? 'border-[#111827]' : 'border-gray-300'}`}>
                                                                {selectedAddressId === addrId && <div className="w-3 h-3 bg-[#111827] rounded-full" />}
                                                            </div>
                        ...
                        <section className={`bg-white rounded-lg shadow-sm border border-gray-100 transition-opacity ${!selectedAddressId ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
                            <div className="bg-gray-50 px-6 py-4 border-b border-gray-100">
                                <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                    <span className="flex items-center justify-center w-6 h-6 bg-[#111827] text-white text-xs rounded-full">2</span>
                                    Payment Method
                                </h2>
                            </div>
                            ...
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    {/* Razorpay Option */}
                                    <div className="border border-gray-200 rounded-lg p-4 hover:border-[#111827] transition-colors">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-3">
                                                <input type="radio" name="payment" id="online" defaultChecked className="text-[#111827] focus:ring-[#111827] w-5 h-5" />
                                        ...
                                        <button
                                            ...
                                            disabled={processing}
                                            className="w-full ml-8 max-w-sm bg-[#111827] text-white font-bold py-3 px-6 rounded hover:bg-[#1f2937] transition-all shadow-md disabled:bg-gray-400"
                                        >
                                    ...
                                    <div className="border border-gray-200 rounded-lg p-4 hover:border-gray-400 transition-colors">
                                        <div className="flex items-center gap-3 mb-3">
                                            <input type="radio" name="payment" id="cod" className="text-[#111827] focus:ring-[#111827] w-5 h-5" />
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-100 sticky top-24">
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div className="border-t pt-4 flex justify-between items-center">
                                    ...
                                    <span className="text-xl font-bold text-[#111827]">₹{totalAmount.toLocaleString()}</span>
                                </div>
                            </div>
